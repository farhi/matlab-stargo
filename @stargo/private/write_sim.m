function cout = write_sim(self,cmd, varargin)
  ...
  case {'abort','full_abort','set_stargo_off'}
    ...
    self.private.get_motors   = [ 1 0 ];
    self.private.get_motor_status = [ 3 3 self.private.zoom-1 ];
  ...
  case 'get_motor_status'
    ...
    if      self.private.sim_move_ra && ~self.private.sim_move_dec, self.get_motor_status(1) = 3;
    elseif ~self.private.sim_move_ra &&  self.private.sim_move_dec, self.get_motor_status(1) = 2;
    elseif ~self.private.sim_move_ra && ~self.private.sim_move_dec, self.get_motor_status(1) = 1;
    else self.get_motor_status(1) = 4; end
    self.get_motor_status(2) = 3;
    self.get_motor_status(3) = self.private.zoom-1;
  case 'get_radec'  
    ...
    if self.private.sim_move_ra
      ...
      if self.private.sim_goto && ...
        ...
        || (self.private.sim_move_ra < 0 && self.private.ra_deg < self.private.sim_target_ra) )
          self.private.get_radec(1) = round(self.private.sim_target_ra*1e6);
          self.private.sim_move_ra  = 0;
      else
        self.private.get_radec(1) = round((self.private.ra_deg + step)*1e6);
      end
    end
    if self.private.sim_move_dec
      step = self.private.sim_move_dec*self.private.dec_speeds(z);
      if self.private.sim_goto && ...
        ( (self.private.sim_move_dec > 0 && self.private.ra_deg > self.private.sim_target_dec) ...
        || (self.private.sim_move_dec < 0 && self.private.ra_deg < self.private.sim_target_dec) )
          self.private.get_radec(2) = round(self.private.sim_target_dec*1e5);
          self.private.sim_move_dec = 0;
      else
        self.private.get_radec(2) = round((self.private.dec_deg + step)*1e5);
      end
    end
    % test for end of GOTO
    if self.private.sim_goto && ~self.private.sim_move_ra && ~self.private.sim_move_dec
      write_sim(self, 'abort'); % set state to TRACKING
    end
  case 'get_motors'
  ...
  case 'set_home_pos' % this sets DEC=90
    self.private.get_radec(2) = 90*1e5;
  end
end % write_sim
